...
class Aql
{
        ...
        foreach ($condition as $key => $value) {
            ...
            if ($key[0] === '|' && is_array($value)) {
                ...
                foreach ($value as $find => $replace) {
                    $key = str_replace($find, $this->quote($replace, $escaper), $key);
                }
                $sql[] = $key;
                continue;
            }
            ...
            if (is_scalar($value) || is_null($value)) {
                $value = $this->quote($value, $escaper);
            }
            else {
                foreach ($value as $k => $v) {
                    $value[$k] = $this->quote($v, $escaper);
                }
            }
    ...
    protected function set(array|null $set, callable $escaper): string
    {
        ...
        $sql = [];
        foreach ($set as $k => $v) {
            $sql[$k] = "`$k` = {$this->quote($v, $escaper)}";
        }

        return ' SET ' . implode(', ', $sql);
    }

    protected function quote(mixed $value, callable $escaper): string
    {
        if (is_null($value)) {
            return 'NULL';
        }
